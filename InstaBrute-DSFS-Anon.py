...
#The main class
class Instabrute():
	def __init__(self, username, passwordsFile='anon.txt'):
	...
	def IsUserExists(self):
		...
		if (r.status_code == 404):
			print ('[*] User bernama "%s" tak ditemukam' % username)
			Input('[*] Tekan enter untuk keluar')
			exit()
...
try:
	delayLoop = int(Input('[*] Masukkan Limit/Jeda (per detik): ')) 
except Exception as e:
	print ('[*] Error, software menggunakan default kode "4"')
	delayLoop = 4
print ('')
